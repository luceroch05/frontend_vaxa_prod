...
import { Users, LogOut, Activity, Calendar, Layers, Globe, Shield, DollarSign, Package, CreditCard } from '@/components/ui/icon';
...
export default function TerapShell() {
  ...
  const nav = [
    ...
    ...(gestiona ? [{ to: terapPath(slug, '/panel/ventas'), label: 'Ventas', icon: DollarSign, end: false }] : []),
    ...(gestiona ? [{ to: terapPath(slug, '/panel/inventario'), label: 'Inventario', icon: Package, end: false }] : []),
    ...(gestiona ? [{ to: terapPath(slug, '/panel/caja'), label: 'Caja', icon: CreditCard, end: false }] : []),
